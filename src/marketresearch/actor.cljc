(ns marketresearch.actor
  "MarketResearchInterviewersActor — the ISCO-08 4227 community marketresearch actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one marketresearch operation request
...
(defn build-graph
  "Build a compiled MarketResearchInterviewersActor graph. `store` implements
  `marketresearch.store/Store`. `advisor` implements
  `marketresearch.advisor/Advisor` (defaults to `mock-advisor`).
